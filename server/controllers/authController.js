...
export const registerController = async (req, res) => {
  try {
    const { name, email, password, phone, company, pan, address, state, city, pin, answer } = req.body;
    //validations
    if (!name) {
    ...
    if (!company) {
      return res.send({ message: "company no is Required" });
    }
    if (!pan) {
      return res.send({ message: "pan no is Required" });
    }
    if (!address) {
